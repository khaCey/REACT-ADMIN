...
function doGet() {
  try {
    ...
    // Retroactive backfill: fetch specific month or full year from Calendar
    if (monthParam && /^\d{4}-\d{2}$/.test(monthParam)) {
      ...
      return jsonOutput_({
        ...
        backfill: { month: monthResult.month }
      });
    }
    if (yearParam && /^\d{4}$/.test(yearParam)) {
      ...
      return jsonOutput_({
        ...
        cacheVersion: 0,
        lastUpdated: new Date().toISOString(),
        backfill: { year: yearResult.year, months: yearResult.months }
      });
    }
    ...
    // Diff mode: return full rows in "updated" for simplicity/idempotent sync.
    return jsonOutput_({
      changed: true,
      diff: {
        added: [],
