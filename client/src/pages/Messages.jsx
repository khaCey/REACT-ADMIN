...
export default function Messages() {
  ...
  const handleSendReply = async () => {
    ...
    setError('')
    try {
      await api.sendMessageItem(selectedConversationId, { body })
      setReplyBody('')
      await Promise.all([
                  ...
                  items.map((item) => (
                    <div key={item.id} className="rounded border border-gray-200 bg-gray-50 px-3 py-2">
                      <div className="flex items-center justify-between gap-2 mb-1">
                        <span className="text-xs font-semibold text-gray-700">{item.sender_name || `Staff #${item.sender_staff_id}`}</span>
                        <span className="text-[11px] text-gray-500">{formatDateTime(item.created_at)}</span>
                      </div>
                      <p className="text-sm whitespace-pre-wrap text-gray-900">{item.body}</p>
                    </div>
                  ))
                )}
              </div>
