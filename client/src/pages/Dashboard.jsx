                        ...
                        segments.map((seg, segIdx) => (
                          <div key={segIdx} className="grid grid-cols-4 gap-1 w-full">
                            ...
                            <article
                              ...
                              className={`dashboard-lesson-card relative h-[50px] rounded border flex items-center justify-between overflow-hidden pl-2 pr-9 ${colSpan} border-gray-200 bg-gray-50 ${hasNote ? 'ring-1 ring-amber-200' : ''} ${lesson.student_id ? 'cursor-pointer hover:bg-white' : ''}`}
                            >
                              <span
                                className={`pointer-events-none absolute top-1 right-1 inline-grid h-5 w-5 place-items-center rounded-full border border-amber-300 bg-amber-50 text-[10px] font-bold text-amber-700 shadow-sm ${hasNote ? '' : 'invisible'}`}
                                aria-hidden="true"
                              >
