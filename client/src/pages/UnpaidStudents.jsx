...
export default function UnpaidStudents() {
  ...
  const fetchStudents = () => {
    api.getUnpaidStudents().then(setStudents).catch((e) => setError(e.message))
  }

  return (
    <div className="w-full flex flex-col">
      <div className="flex justify-between items-center pt-3 pb-2 mb-3 border-b border-gray-200">
        <h2 className="text-2xl font-bold text-gray-900">未納 (Unpaid)</h2>
        <Link
          to="/students"
        ...
        <>
          <div className="relative overflow-auto max-h-[70vh] w-full rounded-xl border border-black/5 bg-white shadow-sm">
            <table className="min-w-full border-separate border-spacing-0">
              <thead className="sticky top-0 bg-amber-600 text-white shadow">
                <tr>
          ...
          <p className="mt-2 text-slate-600 text-sm">
            {students.length} student{students.length !== 1 ? 's' : ''} (未納)
          </p>
        </>
      )}
      {selectedStudentId && (
